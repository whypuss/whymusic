...
**跨子源救援**：單一子源取不到音源時（GD 上游對部分曲目回空字串），後端會用
歌名+歌手到其餘子源找同一首歌。比對前做繁簡歸一化，所以查「浮誇」也能命中
簡體源的「浮夸」。

**未納入 `kuwo` 與 `bilibili`**：實測 `kuwo` 的 url 端點恆回空字串、`bilibili` 回 HTML，
兩者搜得到但播不出來，列進去只會變成啞彈。

**`audiomack` 已移除**：它曾是第三個子音源（歐美獨立音樂 / hip-hop / afrobeats），
但播放不穩 —— URL 解析成功卻常在客戶端播不出來，且疑似有地域限制（同一首歌從不同
地區的 CF 邊緣結果不同）。它獨有的曲目在其餘子源找不到替代，救援機制救不了，
留著只會讓使用者隨機撞到放不出來的歌。專輯搜尋原本只有它提供，因此也一併移除。
...
| 圖示 | 模式 | 行為 |
|------|------|------|
| 🔁 | 自動續播（預設） | 搜尋結果**依序**；推薦頁**隨機**（千首榜單依序播會永遠繞在前幾首） |
...
**換子源重播**：音源給了 URL 但瀏覽器實際播不出來（CDN 對該地區回 403、容器格式
不支援…）時，會把該子源排除後請音源換一個再試同一首歌，最多換兩次。這種失敗
伺服器端看不到 —— 它只知道「解析成功」—— 所以必須由前端回報。
